...
(async () => {
    try {
        ...
        console.log('Query Executed: DROP TABLE IF EXISTS Files');
        console.log('Table "Files" Deleted or already does not exist.');

        // Query to describe the table
        const describeTableQuery = `DESCRIBE Files`;
        const tableStructure = await query(describeTableQuery);
        console.log('Table Structure:');
        console.table(tableStructure);

        // Query to fetch the data in the table
        const selectDataQuery = `SELECT * FROM Files`;
